...
export const Pipeline35: React.FC = () => {
  ...
  return (
    <section id="pipeline" className="py-24 relative bg-[#FCFCFC] dark:bg-[#090D16] border-y border-slate-200/60 dark:border-slate-800 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Engine Marquee */}
        <div className="relative w-full overflow-hidden py-4 mb-16 mask-gradient">
          <div className="absolute left-0 top-0 bottom-0 w-24 bg-gradient-to-r from-[#FCFCFC] dark:from-[#090D16] to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-24 bg-gradient-to-l from-[#FCFCFC] dark:from-[#090D16] to-transparent z-10 pointer-events-none" />
          ...
          <div className="flex gap-4 animate-marquee whitespace-nowrap w-max hover:[animation-play-state:paused]">
            {[...marqueeEngines, ...marqueeEngines].map((engine, idx) => (
              <div
                key={idx}
                className="inline-flex items-center gap-3.5 px-5 py-3.5 rounded-2xl glass-panel-elevated border border-slate-200/80 dark:border-slate-800 shadow-sm hover:border-blue-400/80 transition-all hover:scale-[1.02]"
              >
                <div className="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-900/40 border border-blue-100 dark:border-blue-800 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold shrink-0">
                  <Zap className="w-4 h-4" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-900 dark:text-white text-sm font-sans">{engine.name}</span>
                    <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[10px] font-mono text-slate-600 dark:text-slate-400 font-semibold">
                      {engine.badge}
                    </span>
                  </div>
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">{engine.role}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="glass-panel-elevated rounded-3xl p-6 sm:p-8 border border-slate-200/80 dark:border-slate-800 shadow-lg">
          {/* Filter Tabs */}
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-200/70 dark:border-slate-800">
            <div className="flex flex-wrap items-center gap-2">
              {[
                ...
                { id: "target", label: "1. Target & Pockets (8)" },
                { id: "docking", label: "2. Docking & FEP (9)" },
                { id: "md", label: "3. Local MD Trajectory (8)" },
                { id: "admet", label: "4. ADMET & Safety (10)" },
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            <div className="text-xs font-mono text-slate-500 dark:text-slate-400 hidden sm:flex items-center gap-1.5">
              <Activity className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
              <span>35 / 35 PASS RATE REQUIRED FOR HIT DECLARATION</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-slate-900 dark:text-white font-sans">{test.name}</h4>
                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                          <span className="font-mono text-blue-700 dark:text-blue-400 font-medium">{test.engine}</span>
                          <span>•</span>
                    ...
                    <div className="text-right shrink-0">
                      <span className="inline-block px-2.5 py-1 rounded-md bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-mono font-bold">
                        {test.threshold}
                      </span>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-5 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 sm:p-7 border border-slate-200/90 dark:border-slate-800 shadow-xl flex flex-col justify-between min-h-[460px] relative overflow-hidden bg-gradient-to-b from-white via-white to-blue-50/20 dark:from-slate-900 dark:via-slate-900 dark:to-blue-950/20">
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-500" />

              <div>
                <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 text-xs font-mono text-slate-500 dark:text-slate-400">
                  <span className="flex items-center gap-1.5 text-blue-600 dark:text-blue-400 font-bold">
                    <Info className="w-4 h-4" />
                    TEST GATE #{selectedTest.id < 10 ? `0${selectedTest.id}` : selectedTest.id} SPECIFICATION
                  </span>
                ...
                <div className="pt-4 space-y-4">
                  <div>
                    <span className="text-[10px] uppercase font-mono text-slate-400 dark:text-slate-500 tracking-wider font-semibold">
                      Validation Target
                    </span>
                    <h3 className="text-xl font-extrabold text-slate-900 dark:text-white font-sans mt-0.5">
                      {selectedTest.name}
                    </h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700/60">
                      ...
                      <span className="text-xs font-mono font-bold text-blue-700 dark:text-cyan-300 block mt-1">
                        {selectedTest.engine}
                      </span>
                    </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700/60">
                      ...
                      <span className="text-xs font-mono font-bold text-amber-600 dark:text-amber-300 block mt-1">
                        {selectedTest.gpuTime}
                      </span>
                    </div>
                  </div>

                  <div className="p-4 rounded-2xl bg-emerald-50/70 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60">
                    <span className="text-[10px] uppercase font-mono text-emerald-800 dark:text-emerald-300 block font-bold">
                      Strict Pass Threshold
                    </span>
                    <span className="text-sm font-mono font-extrabold text-emerald-700 dark:text-emerald-400 block mt-1">
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                <span className="flex items-center gap-1">
                  ...
                  Local CUDA/Metal execution verified
                </span>
